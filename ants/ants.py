...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(insect,QueenAnt) and not insect.imposter: return
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        n=0
        while current_place!=hive and n<=self.max_range:
            if len(current_place.bees) and n>= self.min_range:
...
class LongThrower(ThrowerAnt):
    ...
    implemented = True 
    min_range=5
    max_range= 1000000  # Change to True to view in the GUI
    # END Problem 6
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 10
        list_bee=[bee for bee in self.place.bees]
        if self.digesting>0 and list_bee:
            self.digesting-=1

        elif self.digesting==0 and list_bee:
            self.digesting=self.time_to_digest
            self.eat_bee(random_or_none(list_bee))
        else:
            pass
        # END Problem 10
...
# END Problem 13
    ...
    def __init__(self):
        # BEGIN Problem 13
        self.boosted_ants=[]
        if QueenAnt.queen_exists==False:
            self.imposter=False
    ...
    def action(self, colony):
        ...
        if self.imposter:
            ...
            while place.exit:
                place = place.exit
                if place.ant and not (place.ant in self.boosted_ants):
                    place.ant.damage *= 2
                    self.boosted_ants += [place.ant]
                if isinstance(place.ant,BodyguardAnt) and place.ant.ant and not (place.ant.ant in self.boosted_ants):
                    place.ant.ant.damage *= 2
                    self.boosted_ants += [place.ant.ant]
...
def make_slow(action):
    ...
    """
    # BEGIN Problem EC
    def action_slow(colony, *args):
        if colony.time%2==0:
            return action(colony, *args)
        else:
            return None
    return action_slow
    # END Problem EC
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    old_action_method = bee.action
    new_action_method = effect(old_action_method)
    def new_bee_action(*args):
        nonlocal duration
        if duration>0:
            duration -= 1
            new_action_method(*args)
        else:
            old_action_method(*args)
