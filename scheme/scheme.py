...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        initial = scheme_eval(first,env)
        check_procedure(initial)
        return initial.eval_call(rest,env)
        # END PROBLEM 5
...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        curr_frame = self.parent
        while curr_frame is not None:
            for key in curr_frame.bindings:
                if symbol == key:
                    return curr_frame.bindings[key] 
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('Number of arguments do not match')
        else:
            keys = formals
            values = vals
            while keys != nil:
                child.define(keys.first, values.first)
                keys = keys.second
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        arguments = operands.map(lambda y: scheme_eval(y,env))
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN Problem 21
        if operands is nil:
            raise SchemeError("Invalid expression")
        return scheme_eval(complete_eval(scheme_apply(self ,operands, env)), env)
        # END Problem 21
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        returned_val = target.first
        first_part= target.second
        second_part = expressions.second
        make_expr = Pair(first_part,second_part)
        latter = do_lambda_form(make_expr,env)
        env.define(returned_val,latter)
        return returned_val
        # END PROBLEM 10
    else:
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 9
    lambda_instance = LambdaProcedure(formals,expressions.second,env)
...
def do_and_form(expressions, env):
    ...
    while expressions != nil:
        if expressions.second != nil:
            return_val = scheme_eval(expressions.first,env)
        else:
            return_val = scheme_eval(expressions.first, env, True)
        if not scheme_truep(return_val):
            return False
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        i = bindings.first
        check_form(i,2,2)
        if not scheme_symbolp(i.first):
            raise SchemeError
        forms = Pair(i.first, forms)
        formals = Pair(scheme_eval(i.second.first,env),formals)
        bindings = bindings.second
    return env.make_child_frame(forms,formals)
    # END PROBLEM 15
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        name = target.first
        env.define(name, MacroProcedure(target.second, expressions.second, env))
        return name
    else:
        raise SchemeError("Badly formed marco")
    # END Problem 21
